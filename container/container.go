...
func Build(db *gorm.DB, hub *ws.Hub) *AppContainer {
	...
	groupSvc := service.NewGroupService(groupRepo, memberRepo, articleRepo, gCommentRepo, gReactionRepo, userRepo, notifSvc)
	eventSvc := service.NewEventService(eventRepo, agendaRepo, regRepo, userRepo)
